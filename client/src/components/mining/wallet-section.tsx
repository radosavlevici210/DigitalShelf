...
export function WalletWithTransfer({ data = [] }: WalletSectionProps) {
  const { toast } = useToast();
  
  // Mining wallet address
  const walletAddress = "0xe246E8773056bc770A4949811AE9223Bcf3c1A3A";
  const availableBalance = 2.4356; // This would come from API in real implementation
  ...
  return (
    <div className="space-y-4">
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="p-3 bg-secondary/20 rounded-lg">
              <p className="text-xs text-muted-foreground">Total Balance</p>
              <p className="text-lg font-bold text-card-foreground">{availableBalance} ETH</p>
              <p className="text-xs text-muted-foreground">$4,089.23</p>
            </div>
          </div>
      ...
      <WalletTransfer availableBalance={availableBalance} />
    </div>
  );
}
